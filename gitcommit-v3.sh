...
if [[ "$BRANCH_ATUAL" == "main" || "$BRANCH_ATUAL" == "master" || "$BRANCH_ATUAL" == "develop" || "$BRANCH_ATUAL" == release/* ]]; then
    ...
    echo -e "Não é permitido commitar diretamente aqui por regras de governança FBSO."
    echo -e "Por favor, use uma branch de ${GREEN}feature/${NC} ou ${GREEN}hotfix/${NC}.${NC}"
    exit 1
fi
...
# 6. Geração do Timestamp
# Formato solicitado: [YYYYMMDD-HHMMSS]
...
# Estrutura Final: tipo(escopo): mensagem [TAG] [TIMESTAMP]
